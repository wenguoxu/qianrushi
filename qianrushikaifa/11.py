...
def user_login():
    if request.method == 'GET':
        ...
        if user and user['password'] == password:
            return redirect(url_for('jiandannew'))  # 登录成功后跳转到 jiandannew 页面
        else:
            js_code = "<script>alert('登录失败！'); history.back();</script>"
            return js_code

# 路由：管理员页面（登录成功后跳转的页面）
@app.route('/jiandannew')
def jiandannew():
    return render_template('jiandannew.html')  # 跳转到44.html 页面
